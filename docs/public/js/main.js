$(document).ready(function() {
  ...
  timeline.createBreadcrumbs($(".container.full").first());
  timeline.initSiteSearch();
  timeline.initKeywords();

  $("#tipue_search_input").keyup(checkInput)
  timeline.typingTimeout = 0;
      
});

function checkInput() {
  if (document.getElementById("tipue_search_input").value.length > 3) {
    clearTimeout(timeline.typingTimeout);
    timeline.typingTimeout = setTimeout(function () {timeline.initSiteSearch(true)},200);
  }
  else {
 ...
 * @author felixhayashi
 */
timeline.createBreadcrumbs = function(container) {
 ...
 * @author felixhayashi
 */
timeline.initSiteSearch = function(dynamic) { // Added dynamic flag for live update ~ Alex
 ...
 * @author felixhayashi
 */
timeline.initKeywords = function() {
