 ...
 *            All subgroups
 * @param {boolean} shouldStackSubgroups
 */
export function nostack(items, margin, subgroups, shouldStackSubgroups) {
  for (let i = 0; i < items.length; i++) {
    if (items[i].data.subgroup == undefined) {
      items[i].top = margin.item.vertical;
    } else if (items[i].data.subgroup !== undefined && shouldStackSubgroups) {
      let newTop = 0;
