 ...
 */
export function nostack(items, margin, subgroups, stackSubgroups) {
  ...
  if (!stackSubgroups) {
    stackedSubGroups(items, margin, subgroups)
  }
}
 ...
 *            All subgroups
 */
export function stackedSubGroups(items, margin, subgroups) {
  for (const subgroup in subgroups) {
    if (subgroups.hasOwnProperty(subgroup)) {
