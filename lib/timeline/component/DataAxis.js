    ...
    lines.forEach(
      line=> {
        ...
        if (this.options['showMinorLabels'] && isMajor === false) {
          this._redrawLabel(y - 2, line.val, orientation, 'timeline-y-axis timeline-minor', this.props.minorCharHeight);
        }
        if (isMajor) {
          if (y >= 0) {
            this._redrawLabel(y - 2, line.val, orientation, 'timeline-y-axis timeline-major', this.props.majorCharHeight);
          }
        }
        if (this.master === true) {
          if (isMajor) {
            this._redrawLine(y, orientation, 'timeline-grid timeline-horizontal timeline-major', this.options.majorLinesOffset, this.props.majorLineWidth);
          }
          else {
            this._redrawLine(y, orientation, 'timeline-grid timeline-horizontal timeline-minor', this.options.minorLinesOffset, this.props.minorLineWidth);
          }
        }
      });
