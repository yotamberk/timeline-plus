...
/**
 * Retrieve the absolute values of a DOM element
 * @param {Element} elem        A dom element, for example a div
 * @return {number} left        The absolute top/right/bottom/left positions of this element
 *                              in the browser page.
 */
export function getAbsoluteRect(elem) {
...
/**
 * translates the element given x and y values
 * @param {HTMLElement} element
 * @param {number} x
 * @param {number} y
 * @param {boolean} rtl
 */
export function repositionXY(element, x, y, rtl = false) {
  if (x === undefined && y === undefined) return;

  //no y. translate x
  if (y === undefined) {
    element.style.transform = rtl ? `translateX(-${x}px)` : `translateX(${x}px)`;
    return;
  }

  //no x. translate y
  if (x === undefined) {
    element.style.transform = rtl ? `translateY(-${y}px)` : `translateY(${y}px)`;
    return;
  }

  element.style.transform = rtl ? `translate(-${x}px, ${y}px)` : `translate(${x}px, ${y}px)`;
}
